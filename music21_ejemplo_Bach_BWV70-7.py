...
score=m21.corpus.parse('beethoven/opus59no3/movement3.mxl')
print("Beethoven, Cuarteto de cuerdas op.59 no.3 - III Minueto")
eventos=[]
acs=score.chordify()
...
pos={}
pos.update((node,(1,50*index)) for index, node in enumerate(l))
...
print("# comunidades por modularidad:", len(communities))
for i in range(len(communities)):
    print("    comunidad:", i, "  radio:" , nx.radius(G.subgraph(communities[i])), "  diámetro:", nx.diameter(G.subgraph(communities[i])) ,"  # nodos:",len(communities[i]), "  0-nodos:", [n for n in G.subgraph(communities[i]).nodes if n in notas])
for i in range(len(communities)):
